...
class IID_Dataset(Dataset):
    def __init__(self, num, file, choice='train', aug='False', val_sub='CA', opt_term='phi'):
        self.num = num
        self.choice = choice

        self.val_root = '/data_local/ywh/data/DiverseInpaintingDataset/'+val_sub+'/'
        ...
        if self.choice == 'val':
            try:
                filelist = sorted(os.listdir(self.val_root))
                inputlist = filelist[::2]
                labellist = filelist[1::2]

                self.filelist = []
                for i in range(len(inputlist)):
                    self.filelist.append((self.val_root + inputlist[i], self.val_root + labellist[i]))
    ...
    def load_item(self, idx):
        if self.choice == 'train':
            fname1, fname2, fname3 = self.filelist[np.random.randint(0, self.range_max)]
        elif self.choice == 'test':
            fname1, fname2, frame3 = 'demo_input/' + self.filelist[idx], '', ''
        else:
        ...
        if self.choice == 'train':
            ...
            if self.aug == 'True':
                if random.random()>0.5:
#                    alpha = random.random()*0.95+0.05
                    alpha = random.random()*0.5+0.5
                    img = img * alpha + (1-alpha) * ori_img 

        img = img.astype('float') / 255.
        mask = mask.astype('float') / 255.
        ori_img = ori_img.astype('float') / 255.

        return self.transform(img), self.transform(ori_img), self.tensor(mask[:, :, :1]), fname1.split('/')[-1]
...
class IID_Net(nn.Module):
    def __init__(self):
        ...
        self.conv1_1 = nn.Conv2d(18, 32, 3, stride=2, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        pf_x = self.pf_conv(x)
        pat_x = self.extract_pattern(x)

        x = torch.cat([normal_x, bayar_x, pf_x, pat_x], dim=1)
        x = self.conv1_1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.conv2(x)
...
class IID_Model(nn.Module):
    def __init__(self, save_dir='weight/'):
        ...
        #    print(name)
            if name[:22] != 'module.extract_pattern':
        ...
        for name, p in self.gen.named_parameters():
            if name[:22] == 'module.extract_pattern':
                extra_name_list.append(name)
                extra_para_list.append(p)        

        self.gen_optimizer = optim.Adam(para_list, lr=self.lr, betas=(0.9, 0.999))
        self.pat_optimizer = optim.Adam(extra_para_list, lr=self.lr, betas=(0.9, 0.999))

        self.save_dir = save_dir

    def process(self, Ii, Ii_ori, Mg, mask_lambda=0):
        Mo, pattern = self.gen(Ii)
        ...
        if mask_lambda!=0:
            gen_loss += mask_lambda*torch.nn.MSELoss()(pattern, Ii)
...
class InpaintingForensics():
    def __init__(self, args):
        self.train_phi_num = 18000
        self.train_theta_num = 18000
        self.val_num = 1000
        self.test_num = 12
        self.batch_size = 4
        ...
        val_dataset = IID_Dataset(self.val_num, self.val_file, choice='val', val_sub=args.val_sub)

        self.giid_model = IID_Model(args.save_dir).cuda()
        self.n_epochs = 1000
    ...
    def val(self):
        ...
        auc, f1_res, gen_losses = [], [], []

        print(len(self.val_loader))
        for cnt, items in enumerate(self.val_loader):
            Ii, Ii_ori, Mg = (item.cuda() for item in items[:-1])
            filename = items[-1][0]
            Mo, gen_loss = self.giid_model.process(Ii, Ii_ori, Mg)
...
#                f1_res.append(f1_score(Mg.reshape(H * W).astype('int'), Mo.reshape(H * W)) * 100.)
            except:
                print('label zero error!')
                print(items[-1])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--val_sub', type=str, default='GC')
    args = parser.parse_args()

    model = InpaintingForensics(args)
